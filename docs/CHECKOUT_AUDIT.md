...
> **⚠️ Line-number references in this doc are HISTORICAL (frozen 2026-05-15).** `saga.py` has grown from ~500 lines to 1200+; the same bug/behavior may live at a different line today or be fixed and gone entirely. Use this doc for **design context and reasoning**; for current-code references see `docs/PRE_RELEASE_PAYMENT_CHECKLIST.md §4` (per-bug current state) and `docs/ORDER_OPTIMIZER.md` (operational reference).
>
> **Still authoritative here:** the FMEA (§7), the threat-class taxonomy (§5), the gap analyses (§1–§5), and the target architecture (§6). **Stale:** specific `saga.py:N` line numbers throughout, and any `except NotImplementedError:` discussion — those exceptions are deleted; L0–L6 all shipped (see PRE_RELEASE §0 for current state).

---

## 0. Executive summary

> **Status update (2026-05-19):** items 3 + 4 are RESOLVED; items 1, 2, 5, 6, 7 remain open. See `docs/PRE_RELEASE_PAYMENT_CHECKLIST.md §3` for the current roadmap.
...
3. ~~The Saga is not crash-safe.~~ **RESOLVED 2026-05-19 (Phase E step 1):** `scripts/checkout/saga_resume.py:resume_in_flight_sagas()` runs at every lifespan boot and routes every non-terminal saga to a terminal state (FAILED or MANUAL_REVIEW). Closes audit FMEA #3 (RPN 450). State now lives in the Postgres `sagas` table (Phase C, 2026-05-18) rather than a checkpoint file.
4. ~~Stripe is silently bypassed in dev mode.~~ **RESOLVED 2026-05-15 (L0–L5 shipped):** the layered checkout gate refuses to start the Saga unless a `PaymentProvider` is registered and CHECKOUT_ENABLED is true. `except NotImplementedError: pass` blocks deleted; `stripe_client.py` removed.
...
| 21 | **No tests** | No `tests/` directory exists | Refactors break things silently. Pre-launch confidence is low. | 7 | 9 | 3 | **189** → **MITIGATED 2026-05-19** | Section 6.10 minimum coverage shipped: `scripts/test_optimizer.py` (items 1–7), `scripts/test_gate_bypass.py` (item 10), `scripts/test_saga_state_machine.py` (item 8). Item 9 covered-by-integration via `test_phase_e_pg.py` (Phase E.1 saga-resume routing branches). Hot-path acceptance tests (network-fixture LEGO/BrickOwl parse, HAR playback) deferred to roadmap #3/#4 since they require the MarketplaceAdapter refactor to be useful. Effective post-mitigation RPN: ~60 (lower D since regressions in the optimizer / state machine / gate bypass now surface in <1s). |
...
| **L6** Audit log (`audit.emit()` at every Saga state transition) | Design contract in `docs/PRE_RELEASE_PAYMENT_CHECKLIST.md` §2. `payment.skipped` event is the canonical regression signal — should be permanently zero in prod. | ✅ **Shipped 2026-05-19** (`scripts/checkout/audit.py` + 26 emits in `saga.py`; first call site `gate.confirm_rejected` in `dependencies.py`; events table indexed per PRE_RELEASE §8) | Post-hoc detection of any future regression |
...
### Implementation history (collapsed 2026-05-19)

L0–L5 shipped 2026-05-15; L6 shipped Phase E 2026-05-19; Phases A–E.1 of the Postgres migration shipped 2026-05-16 → 2026-05-19. This subsection used to carry:

- the five-pass review findings tagged D1/D2/D3/S1/S2/C1/C2/C3/F1–F6/G1–G3/H1/H2/I1/J1/J2 — all closed,
- the L4 design pre-work / deletion-order / testing plan — the L4 contract now lives in the docstring at the top of `_execute_checkout_saga_inner` in `scripts/checkout/saga.py`,
- the L5 step-by-step migration plan + `StripeProvider` refusal-path test matrix — the live behavior is enforced in `payment/stripe_provider.py:__init__` and verified by `scripts/test_gate_bypass.py`,
- a tiered fix backlog — every Tier 1–3 item shipped.

The "What 'RPN #1 retired' required" checklist (6 items) all shipped. RPN #1 is structurally retired; verified by the 12-assertion `scripts/test_gate_bypass.py`.

For the current shipped state, see `docs/PRE_RELEASE_PAYMENT_CHECKLIST.md §0`. For per-layer source-of-truth contracts, read each module's docstring (`gate.py`, `dependencies.py`, `gate_router.py`, `payment/base.py`, `payment/registry.py`, `payment/stripe_provider.py`, `saga.py`).

---

### L6 — audit log subsystem

**Moved to `docs/PRE_RELEASE_PAYMENT_CHECKLIST.md` §2.** That document is now the design contract for L6 — event envelope, locked vocabulary, storage, retention, public API, migration of call sites, and alerting rules.
