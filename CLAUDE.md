...
- `scripts/db.py` owns the asyncpg pool for the Neon migration (Phase 9). `init_pool()` is called FIRST in Main.py lifespan, BEFORE payment registration / gate / executor; `close_pool()` is called LAST in shutdown, AFTER the registry reset. Both are idempotent and no-op when `DB_BACKEND != postgres` (default `json`). `init_pool()` refuses boot if `DB_BACKEND=postgres` and DATABASE_URL is missing OR points at the Neon DIRECT endpoint instead of the pooler (host suffix `-pooler`). `command_timeout=10s` and `statement_cache_size=0` are non-negotiable for Neon's transaction-mode PgBouncer pooler. Use `is_postgres_backend()` to branch DB-vs-JSON code paths during the staged migration. (Phase A shipped 2026-05-16.)
