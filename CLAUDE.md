...
- Pre-release launch checklist (must-dos for stable payments before any real $1 moves) lives in `docs/PRE_RELEASE_PAYMENT_CHECKLIST.md`. Read it before touching the Saga or doing any go-live work. The prioritized hardening sequence (H1–H15) lives in §8 of that file. The database migration plan (3 rounds + binding decisions log + per-phase playbooks + live progress dashboard) lives in §9 — read in this order: **Terminology block at top of §9** (disambiguates "Neon branch" vs "git branch" — both have a default called `main`) → **§9.6 progress dashboard** (where are we now?) → **§9.5 per-phase playbook** (how to execute the next phase) → §9.4 operating summary → §9.3.11 for resolved decisions (**Neon** chosen as host on 2026-05-16, PG 17.8 us-east-1 ARM64, Free during dev → Launch $19/mo at go-live, Neon `dev` branch for local; hard-switch cutover behind `DB_BACKEND` flag, alembic with raw SQL, periodic DELETE for audit retention).
...
- `is_truthy` (the public env-flag parser) lives in `scripts/checkout/_env.py` (dependency-free; the `_TRUTHY = frozenset({"1", "true", "yes", "on"})` literal is here and ONLY here). `gate.py` re-exports it for backward compatibility (`from .checkout.gate import is_truthy` still works). `payment/registry.py`, `payment/stripe_provider.py`, and `Main.py` all import from `_env` directly. If you add a new env-flag helper, put it in `_env.py` and keep the module dependency-free. Audit check: exactly one `("1", "true", "yes", "on")` literal under `scripts/checkout/`. (H3 shipped 2026-05-16.)
...
- Stripe API key validation (test/live + ≥8 chars beyond prefix) lives in `scripts/checkout/payment/key_format.py` (the `key_mode(key)` function). `gate.py` and `stripe_provider.py` both import it — DO NOT add a third copy. Adding a new key prefix (e.g., `sk_restricted_`) means editing `_PREFIXES` in key_format.py; the rest of the codebase picks it up automatically. (B6/H4 shipped 2026-05-16.)
- `STRIPE_CURRENCY` is read ONCE in `StripeProvider.__init__`, validated against the hardcoded `_ALLOWED_CURRENCIES` frozenset, and exposed as `provider.currency`. The saga reads `provider.currency`, NOT `os.environ.get("STRIPE_CURRENCY", ...)`. The `PaymentProvider` Protocol in `payment/base.py` declares `currency: str` — every future provider implementation MUST set it in `__init__`. Adding a new supported currency requires editing `_ALLOWED_CURRENCIES` and a redeploy (tracked as B27). (B7/H5 shipped 2026-05-16.)
- LEGO ordering has an `except StockoutError` branch above `except Exception` in the saga's LEGO order step. The branch compensates without retry and uses `customer_message: ERROR_MESSAGES["manual_review"]` — DO NOT add a stockout-retry loop for LEGO. LEGO is the primary inventory source; re-routing to LEGO wouldn't help. The branch is currently unreachable because `lego_client.order_from_lego` doesn't yet raise `StockoutError` (tracked as B29 — Option A wiring). (B8/H9 shipped 2026-05-16 as Option B.)
- BrickOwl cancels are parallelized via `_parallel_brickowl_cancels` (saga.py, `Semaphore(5)`). Both sites use it — `_compensate` Phase 1 AND the stockout-retry inline cancel. Individual cancel failures are NOT retried inside the helper today (tracked as B28 — wire per-order retry when the real Playwright cancel ships in roadmap #5). The stockout-retry site waits for ALL cancels (parallel) before deciding MANUAL_REVIEW; do NOT revert to first-failure-aborts behavior. (B16/H10 shipped 2026-05-16.)
- Each marketplace client owns its own cache key naming via `invalidate_listing(element_id)`. The saga does NOT construct cache key strings — it calls `asyncio.gather(brickowl_client.invalidate_listing(eid), lego_client.invalidate_listing(eid), bricklink_client.invalidate_listing(eid))` on stockout. Adding a new marketplace client means adding an `invalidate_listing` mirror; the saga side is automatic. (B9+B10/H8 shipped 2026-05-16.)
- The cache sweep task in `scripts/checkout/cache.py` is held by `_sweeper_task: Optional[asyncio.Task]` (module global, strong reference) to prevent the Python 3.11+ event-loop GC of weakly-referenced tasks. `start_cache_sweeper()` is idempotent — re-calling skips re-start if a non-done task already exists. Same defect class as router.py's `_running_sagas` (C1). Do NOT replace with bare `asyncio.create_task(...)`. (B24 shipped 2026-05-16.)
- `_handle_saga_timeout` MUST always write a terminal state. The load-failure arm now writes a best-effort MANUAL_REVIEW (with verbose `manual_review_reason` runbook) when `checkout_store.load()` raises. The inner write is wrapped in its own try/except so a second failure logs CRITICAL without crashing the timeout coroutine. Do NOT remove the inner safety wrap — it's the "DB unreachable during DB-migration era" insurance. (B25 shipped 2026-05-16.)
- Main.py lifespan shutdown clears the payment provider registry via `payment_registry._reset_for_tests()`. Despite the name, this is safe in production (just clears module-level `_active`). Without it, a second lifespan run in the same Python process (TestClient used twice, future hot-reload tooling) would hit the B17 replacement guard and crash startup. Don't remove it. (H6 shipped 2026-05-16.)
- The router's initial `checkout_store.save()` writes `saga_status=SagaStatus.INITIATED.value` (not the literal string `"pending"`). The pre-2026-05-16 contract that listed `"pending"` as a valid saga_status enum value (B14 docs) is OBSOLETE — `SagaStatus` has no `PENDING` member and the brief window between router save and saga's first checkpoint now correctly serializes as INITIATED. Any frontend code that branches on `"pending"` is dead. (B32 shipped 2026-05-16.)
- `lego_client.order_from_lego` has an explicit `except StockoutError: raise` arm BEFORE the generic `except Exception` wrap. Do NOT consolidate them — the saga's B8/H9 branch relies on `StockoutError` propagating with its original class. Wrapping it as `RuntimeError` (the previous behavior) silently defeats the documented contract. When B29 ships (LEGO DOM stockout detection), the B8 branch becomes reachable automatically. (B33 shipped 2026-05-16.)
- The optimizer's Pass 2 consolidation uses `allocation.get(a_sid, {}).get(eid, 0)` (NOT `allocation[a_sid][eid]`) when reading another seller's allocated quantity. The `.get()` chain matches Pass 1's pattern (see comment at optimizer.py:95-96) and prevents defaultdict ghost entries. Bare `allocation[a_sid][eid]` reads create zero-quantity ghosts that pollute `AllocationEntry.items` and would crash `brickowl_client.create_order` when it ships (roadmap #5). Don't revert. (B34 shipped 2026-05-16.)
- `debug_optimize` in `debug_router.py` MUST match the saga's quote flow exactly: include all three marketplace clients in `merge_listings(...)` AND wrap with `apply_free_shipping_thresholds(...)`. If the saga's quote flow changes (e.g., adds another optimizer pass), `debug_optimize` needs the same update — otherwise operators previewing prices see different totals than customers. (B37/B38 shipped 2026-05-16.)
- Main.py's L1 boot block uses `key_format.key_mode(stripe_key) == "live"` (the canonical strict check) — NOT `stripe_key.startswith("sk_live_")`. All three defense-in-depth layers (L0 gate, L1 boot, L5 provider) classify keys identically. Don't introduce a fourth divergent check. (B39 shipped 2026-05-16.)
- `scripts/db.py` owns the asyncpg pool for the Neon migration (Phase 9). `init_pool()` is called FIRST in Main.py lifespan, BEFORE payment registration / gate / executor; `close_pool()` is called LAST in shutdown, AFTER the registry reset. Both are idempotent and no-op when `DB_BACKEND != postgres` (default `json`). `init_pool()` refuses boot if `DB_BACKEND=postgres` and DATABASE_URL is missing OR points at the Neon DIRECT endpoint instead of the pooler. Hostname check uses `urllib.parse.urlsplit` (NOT substring-match on the full DSN, so a password containing `-pooler` doesn't false-trigger). `_pool` is assigned only AFTER the `SELECT 1` connectivity smoke succeeds (atomicity fix). `command_timeout=10s` and `statement_cache_size=0` are non-negotiable for Neon's transaction-mode PgBouncer pooler. A JSONB type codec (`json.dumps`/`json.loads`, `format='text'`) is registered on every connection via the `init=` callback — callers pass/receive Python dict/list for JSONB columns transparently. Use `is_postgres_backend()` to branch DB-vs-JSON code paths during the staged migration. (Phase A shipped 2026-05-16, hardenings in Phase B retro 2026-05-17, JSONB codec 2026-05-18.)
- **Branch terminology — read this before any DB-migration discussion.** "branch" is overloaded. **git branch** = VCS branch in the LAIGO repo. **Neon branch** = copy-on-write database clone in Neon (independent concept). Both systems have a default branch called `main`. When `docs/PRE_RELEASE_PAYMENT_CHECKLIST.md` §9 says "the `dev` branch off `main`" the context is Neon (Neon dev branch off Neon main); when a PR says "merge to main" the context is git. The terminology block at the top of §9 is canonical — qualify every "branch" / "main" reference as "Neon branch" / "Neon `main`" or "git branch" / "git `main`" whenever the surrounding context doesn't make it self-evident.
- `scripts/jobs_db.py` owns Phase D-foundation shadow writes to the `jobs` table at the 5 lifecycle points (insert at /generate, update at scheduler dispatch, terminal writes from done-callback and watchdog, delete at cleanup TTL). Each write goes through `_exec_under_lock(job_id, sql)` — `pg_advisory_xact_lock(hashtextextended(job_id, 0))` serializes the four UPDATEs per job_id at the DB level (B41 fix). Terminal writes (mark_complete/failed/timed_out) use `started_at = COALESCE(started_at, NOW())` to backfill in case mark_running matched 0 rows. All writes are best-effort with try/except; in-memory `app.state.jobs` remains the runtime source of truth until Phase D step 2 collapses the two stores. `mark_*_from_thread` wrappers submit coroutines to the FastAPI event loop via `asyncio.run_coroutine_threadsafe` (the loop is captured in lifespan via `set_event_loop`). Add a new lifecycle write only by adding both an async fn and a `*_from_thread` wrapper, and route through `_exec_under_lock` so the per-job_id serialization invariant is preserved. (Phase D-foundation shipped 2026-05-18; B41 fix shipped 2026-05-18.)
- `scripts/checkout/checkout_store_dispatch.py` is the import target for `saga.py`, `router.py`, `debug_router.py` — re-routes load/save/update calls to either `checkout_store.py` (JSON, default) or `checkout_store_pg.py` (Postgres) based on `DB_BACKEND` checked PER CALL (runtime, not import-time). `read_order_list` is always filesystem-backed (artifact, not state). `ActiveCheckoutExistsError` lives in `checkout_store.py` (canonical) and is re-exported through the dispatcher; the Postgres backend raises it on `sagas_one_active_per_job_idx` B23 violation, the router catches and returns 422 with `code="ACTIVE_CHECKOUT_EXISTS"`. JSON backend does NOT raise it (B23 is OPEN for json mode; tracked as B47). Do NOT import from `checkout_store.py` or `checkout_store_pg.py` directly outside the dispatcher and the modules themselves — that bypasses the runtime check. (Phase C shipped 2026-05-18.)
- `scripts/checkout/checkout_store_pg.py` is the Postgres backend. `save()` INSERTs `checkouts` row FIRST then `sagas` row (FK ordering matters), via the `_insert_checkouts` and `_insert_sagas` helpers. Both helpers list EVERY column from §9.2.1 explicitly in their INSERT VALUES list (B45 fix shipped 2026-05-18) — there is NO trailing UPDATE pattern, so schema-evolution drift fails fast at runtime instead of silently corrupting state. `update()` uses Pattern 2 (`pg_advisory_xact_lock` + `SELECT FOR UPDATE` + merge + UPDATE in one transaction) and is the only function that still uses `_build_update_clause` (legitimate partial-update). State-key → column mapping is `_CHECKOUT_COLS` / `_SAGA_COLS`; rename `error <-> error_message` is the only column-renaming case. `_warn_unknown_keys` (B46) logs WARN when a state dict contains keys not in `_CHECKOUT_COLS ∪ _SAGA_COLS` (typo trap). The asyncpg JSONB type codec from `db.py` handles `checkouts.allocation`, `checkouts.unsourceable_items`, and `sagas.brickowl_order_ids` transparently — do NOT json.dumps these in callers.
- **Schema evolution discipline (B45 contract; applies to all DB-backed write paths including the future Phase D step 2 jobs writes).** When adding a column to `checkouts`, `sagas`, or any new table that follows the all-columns-explicit pattern: (a) **NOT NULL with a DB-side DEFAULT** — INSERTs still work because the DB applies the default. Update `_CHECKOUT_COLS`/`_SAGA_COLS` IF the column should be readable/writable via `update()`. Adding to the INSERT VALUES list is optional but recommended. (b) **NOT NULL without a DEFAULT** — REQUIRED to update the explicit INSERT helper (`_insert_checkouts`, `_insert_sagas`, or the Phase D step 2 analog for `jobs`) AND the column map in the SAME PR as the migration. The migration alone will crash production at the first write to that table. (c) **Nullable** — INSERTs default to NULL via the explicit VALUES list (existing code uses `state.get("...", None)` which produces NULL). Update the column map if you want `update()` to write it. **Pattern reuse:** Phase D step 2's `jobs` lifecycle writes will follow this same rule — see `_insert_checkouts`/`_insert_sagas` as the template.
- **Open DB-migration defects (2026-05-18 inventory, post-hardening batch):** B11, B23 (json mode), B26, B35, B36, B42, B44 are subsumed by future phases (D step 2 / E / F). B45, B48, B49, B51, B52 are cleanup work post-Phase-C. None are launch-blocking. Shipped 2026-05-18 (hardening batch): B43 (alembic-head boot assertion in `scripts/db.py:verify_alembic_head_matches_expected`, called from lifespan unconditionally), B46 (`_warn_unknown_keys` in checkout_store_pg surfaces typos), B47 (L1 boot invariant (c) refuses `CHECKOUT_ENABLED=true + DB_BACKEND=json`), B50 (smoke_test_db.py imports psycopg2 + checks alembic_version row vs `_EXPECTED_SCHEMA_VERSION`). See PRE_RELEASE §4 for each.
- **Boot invariants enforced in `scripts/Main.py` lifespan (in order):** (1) `verify_alembic_head_matches_expected()` — B43 code-time drift, runs unconditionally; (2) `init_pool()` — B44 DB connectivity if `DB_BACKEND=postgres`; (3) `verify_schema()` — B5 deploy-order, runs if postgres; (4) L1 (a) CHECKOUT_ENABLED + gate=DISABLED refuses boot; (5) L1 (b) live Stripe key outside Render refuses boot; (6) L1 (c) CHECKOUT_ENABLED + DB_BACKEND≠postgres refuses boot (B47 fix); (7) executor + scheduler + cleanup threads + cache sweeper start. ALL boot-time refusals raise `RuntimeError` after `log.critical(...)` — fail loud, fail early.
