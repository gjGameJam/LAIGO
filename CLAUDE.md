...
- Reads env: `LEGO_SHIPPING_COST_CENTS` (599), `LEGO_MAX_QTY_PER_ITEM` (9999), `LEGO_FALLBACK_TIMEOUT_SECONDS` (90), `OUTPUT_DIR`, `LEGO_BROWSER_CDP_URL` (unset → launch locally), `LEGO_PLAYWRIGHT_HEADLESS` (default `true`). **Deprecated:** `LEGO_EMAIL`, `LEGO_PASSWORD` are still read into module globals but no longer used by the ordering flow — replaced by `storage_state` caching via `lego_session_store`. Kept readable because `gate.compute_decision()` still consults them; do not rely on them for ordering.
...
- **Checkout-flow status (2026-06-11):** the upload→cart→checkout selectors (`_run_checkout` Steps B–D) are **verified against live PaB and ported**. Step E (payment/place-order) is **gated** (real charge — do NOT wire). Re-verify selectors with `scripts/checkout/_inspect_pab.py` (headed) if the SPA drifts. Full step-by-step selector map: `docs/ORDER_OPTIMIZER.md §6.1`.
- **Cloudflare prod blocker:** Cloudflare hard-blocks **headless** Chromium on Pick-a-Brick (stealth drivers incl. patchright all fail; only **headed** passes — fingerprint problem, not CAPTCHA). The launch is env-driven: `LEGO_BROWSER_CDP_URL` → `connect_over_cdp` to a remote headed browser; else local launch gated by `LEGO_PLAYWRIGHT_HEADLESS`. **Chosen fix: VPS + residential-proxy headed browser over CDP** — design + runbook in `docs/LEGO_BROWSER_HOST.md` (next step: provision). LEGO ordering cannot run on Render until that host exists.
...
- **LEGO browser host (Cloudflare bypass):** `docs/LEGO_BROWSER_HOST.md` — why headless fails, the VPS + residential-proxy headed-browser-over-CDP design, security model, docker-compose, setup runbook, and the provider decisions still needed before provisioning.
- **End-to-end checkout completion plan:** `docs/CHECKOUT_COMPLETION_PLAN.md` — the full remaining-work spec to automate payment (frontend Stripe UI) → LEGO place-order → customer email + instructions-PDF delivery. Workstreams A–E, dependency-ordered milestones, open decisions, and a launch checklist. Start here to resume the checkout build.
